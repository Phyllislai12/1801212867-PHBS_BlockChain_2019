...
public class TestCases {
    ...
    public void testCase1() throws NoSuchAlgorithmException, SignatureException, InvalidKeyException {
        ...
        //init genesis block
        Block genesisBlock = new Block("genesisPreviousHash".getBytes(), users.get(0).getPublic());
        ...
        block1B.finalize();
        // add to block chain
        blockHandler.processBlockManualFork(block1B);
        ...
        block3B.finalize();
        // add to block chain
        blockHandler.processBlockManualFork(block3B);

        // block 3,B, processBlockManualFork
        Block block3BB = new Block(block2M.getHash(), users.get(5).getPublic());
        block3BB.finalize();
        // add to block chain
        blockHandler.processBlockManualFork(block3BB);
...
//
    ...
    public void testCase3() throws NoSuchAlgorithmException, SignatureException, InvalidKeyException {
        ...
        block5M.finalize();
        // add to block chain
        blockHandler.processBlockManualFork(block5M);
...
//
    ...
    @Test
    public void testBlockChainInvalidPreviousBranch() throws Exception{
        //init ten players in the block chain
        ArrayList<KeyPair> users = initKeyPairs(10);
        //init genesis block
        Block genesisBlock = new Block("genesisPreviousHash".getBytes(), users.get(0).getPublic());
        ...
        Transaction tx0 = initTx_oneInOneOut(genesisBlock.getCoinbase().getHash(),0,
                10,0,users.get(1).getPublic(),users.get(0).getPrivate());
        // init block
        Block oloOne = new Block(genesisBlock.getHash(), users.get(1).getPublic());
        oloOne.addTransaction(tx0);
        oloOne.finalize();
        // add block to the block chain
        blockHandler.processBlock(oloOne);

        Block newOne = new Block(new String("Invalid").getBytes(), users.get(1).getPublic());
        newOne.addTransaction(tx0);
        newOne.finalize();
        // add block to the block chain
        blockHandler.processBlock(newOne);

        System.out.println(blockHandler.getBlockChain().getBlocks().containsValue(newOne));
